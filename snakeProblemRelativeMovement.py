...
class SnakePlayer(list):
    ...
    def sense_food_ahead(self):
        ...
        head = self.body[0]
        if self.direction == S_UP:
            return food[0] <= head[0] and food[1] == head[1]
        elif self.direction == S_RIGHT:
            return food[0] == head[0] and food[1] >= head[1]
        elif self.direction == S_DOWN:
            return food[0] >= head[0] and food[1] == head[1]
    ...
    def sense_tail_right(self):
        if(self.direction == S_UP):
            right_pos = [self.body[0][0], self.body[0][1]+1]
        elif(self.direction == S_RIGHT):
            right_pos = [self.body[0][0]+1, self.body[0][1]]
        elif(self.direction == S_DOWN):
            right_pos = [self.body[0][0], self.body[0][1]-1]
        else:
            right_pos = [self.body[0][0]-1, self.body[0][1]]
        return right_pos in self.body

    def sense_tail_left(self):
        if(self.direction == S_UP):
            left_pos = [self.body[0][0], self.body[0][1]-1]
        elif(self.direction == S_RIGHT):
            left_pos = [self.body[0][0]-1, self.body[0][1]]
        elif(self.direction == S_DOWN):
            left_pos = [self.body[0][0], self.body[0][1]+1]
        else:
            left_pos = [self.body[0][0]+1, self.body[0][1]]
        return left_pos in self.body
    ...
    # TODO: modify sense two functions
...
pset.addTerminal(snake.moveForward)  # Terminals are snake movements
pset.addTerminal(snake.moveLeft)
pset.addTerminal(snake.moveRight)
        ...
        if numDecisionGraphs > 0:
            ...
            for i in range(numRuns):
                ...
                for j in range(numDecisionGraphs):
                    filename = "decision_rel/run_" + str(i) + "_num_" + str(j) + ".pdf"
